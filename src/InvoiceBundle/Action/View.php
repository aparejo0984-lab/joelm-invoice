...
final class View
{
    ...
    public function __invoke(Request $request, Invoice $invoice)
    {
        if ('pdf' === $request->getRequestFormat() && $this->pdfGenerator->canPrintPdf()) {
            $template = $this->twig->render('@SolidInvoiceInvoice/Pdf/invoice.html.twig', ['invoice' => $invoice]);
            return new PdfResponse($this->pdfGenerator->generate($template), "Invoice 10{$invoice->getId()}.pdf");
        }
