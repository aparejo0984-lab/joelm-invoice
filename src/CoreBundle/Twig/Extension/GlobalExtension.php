...
class GlobalExtension extends AbstractExtension implements GlobalsInterface
{
    ...
    public function getFunctions()
    {
        return [
            ...
            new TwigFunction('app_ecra_logo', function (Environment $env, string $width = 'auto'): string {
                return $this->displayEcraLogo($env, $width);
            }, ['is_safe' => ['html'], 'needs_environment' => true]),

            new TwigFunction('app_white_logo', function (Environment $env, string $width = 'auto'): string {
                return $this->displayWhiteAppLogo($env, $width);
            }, ['is_safe' => ['html'], 'needs_environment' => true]),
     ...
     * @throws InvalidArgumentException|ServiceCircularReferenceException|ServiceNotFoundException|LoaderError|SyntaxError
     */
    public function displayWhiteAppLogo(Environment $env, string $width = 'auto'): string
    {
        $logo = self::DEFAULT_LOGO;

        if ($this->installed) {
            try {
                $logo = $this->systemConfig->get('system/company/white_logo');
     ...
     * @throws InvalidArgumentException|ServiceCircularReferenceException|ServiceNotFoundException|LoaderError|SyntaxError
     */
    public function displayEcraLogo(Environment $env, string $width = 'auto'): string
    {
        $logo = self::DEFAULT_LOGO;

        if ($this->installed) {
            try {
                $logo = $this->systemConfig->get('system/company/ecra_logo');
